...
//    }
    ...
    for (int i = 0; i < num_samples; ++i)
    {
        ...
        mutation_tester(ind1_cpy);
        mutation_tester(ind2_cpy);
...
//            series0->setMarkerSize(std::log(double(counts[i][j]*10000000/num_samples)));
            series0->setMarkerSize(counts[i][j]*100/num_samples);
            *series0 << QPointF(i,j);
    ...
    chart->setTitle("Testing Integer Crossover and Mutation");
